...
public class MazePanel extends JPanel implements Runnable {
    ...
    private Stroke edgeStroke = new BasicStroke(5F);
	...
	Dimension dim = new Dimension(cellWidth * numCols, cellWidth * numRows);
	...
	for (Edge edge : maze) {
	    g2.drawLine(edge.getU().getCol() * cellWidth + cellCenterOffset,
		    edge.getU().getRow() * cellWidth + cellCenterOffset, edge
			    .getV().getCol() * cellWidth + cellCenterOffset,
